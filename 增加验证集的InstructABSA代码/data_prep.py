...
class DatasetLoader:
    def __init__(self, train_df_id=None, test_df_id=None, train_df_ood=None, test_df_ood=None, sample_size=1, eval_df_id=None):
        
        self.train_df_id = train_df_id.sample(frac=sample_size, random_state=1999) if train_df_id is not None else None
        self.test_df_id = test_df_id
        self.eval_df_id = eval_df_id
    ...
    def set_data_for_training(self, tokenize_function):
        """
        Create labels and text data for fine-tuning and evaluation.
        """
        id_dict = {}
        if self.train_df_id is not None:
            id_dict['train'] = Dataset.from_pandas(self.train_df_id)
        if self.test_df_id is not None:
            id_dict['test'] = Dataset.from_pandas(self.test_df_id)
        if self.eval_df_id is not None:
            id_dict['eval'] = Dataset.from_pandas(self.eval_df_id)
        
        ood_dict = {}
        if self.train_df_ood is not None:
            ood_dict['train'] = Dataset.from_pandas(self.train_df_ood)
        if self.test_df_ood is not None:
            ood_dict['test'] = Dataset.from_pandas(self.test_df_ood)

        id_ds = DatasetDict(id_dict)
        ood_ds = DatasetDict(ood_dict)

        id_tokenized_ds = id_ds.map(tokenize_function, batched=True)
        ood_tokenized_ds = ood_ds.map(tokenize_function, batched=True)
